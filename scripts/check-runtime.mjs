...
for (const target of ['brave', 'chrome', 'edge', 'firefox']) {
  ...
  assert.match(mainBundle, /leetgitsync-bridge-v2/);
  assert.doesNotMatch(mainBundle, /BRIDGE_CONNECTED/);
  ...
  const context = vm.createContext({ [target === 'firefox' ? 'browser' : 'chrome']: api, console, URL, URLSearchParams, AbortSignal, TextEncoder, TextDecoder,
  ...
  const saved = await send({ type: 'SAVE_CONFIG', data: { config: values, confirmPending: false } });
  ...
  assert.equal((await send({ type: 'LEETGITSYNC_DIAGNOSTIC', data: { stage: 'PAGE_OBSERVER_READY', slug: 'two-sum' } }, contentSender)).success, false);
  const productionState = await send({ type: 'GET_STATE' });
