...
export function App({ popup = false }) {
  ...
  return <main className={`app ${popup ? 'popup' : 'options'}`}>
    ...
    <Card className="panel">
      ...
      {!!state?.queue?.contestHeldCount && <p className="notice">Contest hold: {state.queue.contestHeldCount} accepted solution(s) saved locally. GitHub publishing is paused until the contest ends.{state.queue.contestReleaseAt ? ` Automatic release after ${new Date(state.queue.contestReleaseAt).toLocaleString()}.` : ''}</p>}
      {!!state?.queue?.contestUnknownCount && <p className="notice error">Contest end time could not be verified for {state.queue.contestUnknownCount} solution(s). They remain held and will not be published.</p>}
